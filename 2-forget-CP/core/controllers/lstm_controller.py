...
class MyLSTMCell(nn.LSTMCell):
    def __init__(self, input_size, hidden_size, bias=True):
        ...
        if bias:
            ...
            n = self.bias_ih.size(0)
            start, end = n//4, n//2
            self.bias_ih.data[start:end].fill_(1.)

            self.bias_hh = nn.Parameter(torch.Tensor(4 * hidden_size))
            n = self.bias_hh.size(0)
            start, end = n//4, n//2
            self.bias_hh.data[start:end].fill_(1.)
            """
            self.bias_hh2 = nn.Parameter(torch.Tensor(4 * hidden_size))
            self.bias_hh3 = nn.Parameter(torch.Tensor(4 * hidden_size))
            self.bias_hh4 = nn.Parameter(torch.Tensor(4 * hidden_size))
            self.bias_hh5 = nn.Parameter(torch.Tensor(4 * hidden_size))
            """
        else:
