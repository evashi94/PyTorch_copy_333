...
#my steep sigmoid gate biased MyLSTMCell version(paramter alpha)

class MyLSTMCell(nn.LSTMCell):
    def __init__(self, input_size, hidden_size, alpha=4, bias=True):
    ...
    def forward(self, input, hx):
        ...
        i = F.sigmoid(self.alpha*(wx[:, :self.hidden_size] + wh[:, :self.hidden_size]))
        f = F.sigmoid(self.alpha*(wx[:, self.hidden_size:2 * self.hidden_size] + wh[:, self.hidden_size:2 * self.hidden_size]))
        g = F.tanh(wx[:, 2 * self.hidden_size:3 * self.hidden_size] + wh[:, 2 * self.hidden_size:3 * self.hidden_size])
        o = F.sigmoid(self.alpha*(wx[:, 3 * self.hidden_size:] + wh[:, 3 * self.hidden_size:]))
        c_new = f * hx[1] + i * g
